...
echo "$($PSQL "SELECT name FROM games left join teams on games.winner_id = teams.team_id where year='2014' and round='Eighth-Final' union all SELECT name FROM games left
join teams on games.opponent_id = teams.team_id where year='2014' and round='Eighth-Final' order by name")"
...
echo "$($PSQL "SELECT name FROM games left join teams on games.winner_id = teams.team_id where name like 'Co%' order by name")"
